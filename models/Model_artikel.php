...
class Model_artikel extends CI_model
{
    ...
    {
        return $this->db->query("SELECT * FROM tb_blog_artikel left join tb_pengguna on tb_blog_artikel.username=tb_pengguna.nama_lengkap left join tb_blog_kategori on tb_blog_artikel.id_kategori=tb_blog_kategori.id_kategori where tb_blog_artikel.utama='Y' ORDER BY id_artikel DESC LIMIT 0,$limit");
    }
    ...
    {
        return $this->db->query("SELECT * FROM tb_blog_artikel a LEFT JOIN tb_pengguna b ON a.username=b.nama_lengkap LEFT JOIN tb_blog_kategori c ON a.id_kategori=c.id_kategori where a.id_artikel='" . $this->db->escape_str($id) . "' OR a.judul_seo='" . $this->db->escape_str($id) . "'");
    }
